...
def train(agent,
    ...
    # init expert
    qnetwork_star = QNetwork(state_size=6, action_size=3, seed=0)
    qnetwork_star.load_state_dict(torch.load('models/dqn_14.pth'))
    qnetwork_star.eval()
    Q_threshold = 0.0
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        for t in range(max_t):
            ...
            with torch.no_grad():
                ...
                Q_values = qnetwork_star(state_t).data.numpy()
        ...
        scores.append((score, assist_count))
        eps = max(eps_end, eps_decay*eps) # decrease epsilon
        print('\rEpisode {}\tAverage Score: {:.2f}'.format(i_episode, np.mean(scores_window)), end="")
        if i_episode % 100 == 0:
            savename = "models/assist_full_" + str(savenumber) + ".pth"
...
if __name__ == "__main__":
    ...
    scores = train(agent)
    pickle.dump( scores, open( "results/assist_full.pkl", "wb" ) )
