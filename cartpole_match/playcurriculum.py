...
def rollout(Q_threshold, data):
    ...
    for timestep in range(200):
        ...
        if len(humandata) > 0:
            idx = np.linalg.norm(humandata[:,0:4] - state, axis=1).argmin()
            action = int(humandata[idx,4])
...
def experience_matching():
    ...
    for count in range(n_episodes):
        ...
        for item in experiences:
            state, loss, accepted, action, action_star = item[0:4], item[4], item[5], item[6], item[7]
            if not accepted:
                corrections += 1
            if loss > 0.5:
